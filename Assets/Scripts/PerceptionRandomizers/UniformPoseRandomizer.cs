...
public class UniformPoseRandomizer : Randomizer {
  /*  Randomizes object's position and rotation relative to a fixed starting
   * pose, over the specified range.
   *
   *  Example use-case:
   *  Make very small random adjustments to the camera's pose, to make the
   * learned model more robust to small inaccuracies in placing the real camera.
   */
  ...
  protected override void OnIterationStart() {
    IEnumerable<UniformPoseRandomizerTag> tags =
        tagManager.Query<UniformPoseRandomizerTag>();
    foreach (UniformPoseRandomizerTag tag in tags) {
      Vector3 adjustedPosition =
          AdjustedVector(tag.rootPosePosition, positionRange);
      Vector3 adjustedRotation =
          AdjustedVector(tag.rootPoseRotation, rotationRangeDegrees);
