...
public class PlacementConstraint {
  ...
  public virtual bool Passes(float placementX, float placementZ,
                             float objectRadius) {
    return true;
  }
}
...
public class CollisionConstraint : PlacementConstraint {
  ...
  public override bool Passes(float placementX, float placementZ,
                              float objectRadius) {
...
public class SurfaceObjectPlacer {
  ...
  private List<PlacementConstraint> collisionConstraints =
      new List<PlacementConstraint>();

  public SurfaceObjectPlacer(GameObject plane, FloatParameter random,
  ...
  public bool PlaceObject(GameObject obj, bool respectMaxRobotReachability) {
    ...
    if (obj.activeInHierarchy) {
      ...
      List<PlacementConstraint> constraints =
          GetAllConstraints(respectMaxRobotReachability);
      Vector3? point = SampleValidGlobalPointOnPlane(
          radius, constraints, planeBounds, respectMaxRobotReachability);

      if (point.HasValue) {
        // place object
        Vector3 foundPoint = point ?? Vector3.zero;
        obj.transform.position = new Vector3(
            foundPoint.x, foundPoint.y + heightAbovePlane, foundPoint.z);

        // update constraints so subsequently placed object cannot collide with
        // this one
  ...
  private Vector3
      ? SampleValidGlobalPointOnPlane(float objectRadius,
                                      List<PlacementConstraint> constraints,
                                      Bounds planeBounds,
                                      bool respectMaxRobotReachability) {
  ...
  private List<PlacementConstraint>
  GetAllConstraints(bool respectMaxRobotReachability) {
    // return a list of all the constraints: combination of permanent constraint
    // and additional constraint like the maxReachabilityConstraint or the
  ...
  private Vector3 SampleGlobalPointOnPlane(float minEdgeDistance,
                                           Bounds planeBounds,
                                           bool respectMaxRobotReachability) {
    Rect planePlacementZone = PlanePlacementZone(planeBounds, minEdgeDistance);
    if (respectMaxRobotReachability) {
      Rect withinMaxReachZone =
          MaxReachabilityPlacementZone(maxRobotReachability);
    ...
    Vector3 globalPt = new Vector3(randomPlanePoint.x, planeBounds.center.y,
                                   randomPlanePoint.y);
    return globalPt;
  }

  private static Rect
  MaxReachabilityPlacementZone(ReachabilityConstraint maxRobotReachability) {
    float x = maxRobotReachability.robotX -
              maxRobotReachability.robotReachabilityLimit;
    float z = maxRobotReachability.robotZ -
              maxRobotReachability.robotReachabilityLimit;
    float size = maxRobotReachability.robotReachabilityLimit * 2;
    return new Rect(x, z, size, size);
  }

  private static Rect PlanePlacementZone(Bounds planeBounds,
  ...
  private static bool PassesConstraints(Vector3 point, float objectRadius,
                                        List<PlacementConstraint> constraints) {
