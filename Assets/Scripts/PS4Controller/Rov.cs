...
public class rov : MonoBehaviour {
  ...
  void Awake() // called even before start
  {
    ...
    controls.Gameplay.Rotate.performed += ctx => rotate =
  ...
  void Update() {
    ...
    // * 50f *
    Vector2 r = 10f * Time.deltaTime *
                new Vector2(rotate.y, -rotate.x); // 100f to make it quicker
    transform.Rotate(r, Space.Self);              // the world space
    // GetComponent<Rigidbody>().MovePosition
  }

  void MoveUp() { transform.Translate(0, -0.05f * Time.deltaTime, 0); }

  void MoveDown() { transform.Translate(0, 0.05f * Time.deltaTime, 0); }
